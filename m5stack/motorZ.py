...
async def send_http_request_until_success():
    while True:
        try:
            response = urequests.get("http://" + ip + "/op=start_z")
            response_json = response.json()
            response.close()
            
            if response_json.get("status") == 1:
                z_speed = response_json.get("zspeed", 100)  # Velocità del motore stepper (default 100)
                z_acc = response_json.get("zacc", 1)  # Microstep (default 1)
                z_delay = response_json.get("zdelay", 0.001)  # Delay (default 0.001)
                print(f"Risposta HTTP ricevuta. Velocità: {z_speed}, Microstep: {z_acc}, Delay: {z_delay}")
                configure_motor_parameters(z_speed, z_acc, z_delay)
                return z_speed, z_acc, z_delay
            else:
...
async def main_loop():
    while True:  # Simula il loop() di Arduino
        try:
            ...
            z_speed, z_acc, z_delay = await send_http_request_until_success()
